...
3. **Install the dependencies**, from the `Backend` folder:
   ```powershell
...
`HANA` folder — the one that contains `Backend`, not from inside `Backend` itself — because the
code refers to itself as `Backend.app...`, which only resolves correctly one level up:
